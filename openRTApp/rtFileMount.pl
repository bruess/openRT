...
# Check if we need to use a custom agents path (for non-standard pools)
my $agents_dataset;
if ($ENV{RT_AGENTS_PATH}) {
    $agents_dataset = "$rt_pool/" . $ENV{RT_AGENTS_PATH};
    debug("Using custom agents path: $agents_dataset");
} else {
    $agents_dataset = "$rt_pool/home/agents";
    debug("Using default agents path: $agents_dataset");
}
