...
sub find_rt_pool {
    # Define pool name patterns to look for
    my @pool_patterns = (
        qr/^rtPool-\d+$/,   # Original rtPool-123 pattern
        qr/^revRT/          # revRT pattern
    );
    ...
    # Check for environment variable that could override patterns
    if ($ENV{RT_POOL_PATTERN}) {
        my $custom_pattern = $ENV{RT_POOL_PATTERN};
        unshift @pool_patterns, qr/$custom_pattern/;
    }
    
    # First check if there's an exact match via RT_POOL_NAME environment variable
    if ($ENV{RT_POOL_NAME} && grep { $_ eq $ENV{RT_POOL_NAME} } @pools) {
        return $ENV{RT_POOL_NAME};
    }
    
    # Try each pattern in order
    foreach my $pattern (@pool_patterns) {
        foreach my $pool (@pools) {
            return $pool if $pool =~ $pattern;
        }
    }
    
    # If only one pool exists, use it as a fallback
    return $pools[0] if scalar(@pools) == 1;
